...
export class RentalDAO extends BaseDAO<Rental> {
  ...
  async findOverLapping(
    ...
    const query = runner(this.tableName)
  ...
  async update(
    ...
    const runner = trx || this.db;
    const [rental] = await runner(this.tableName)
      .where({ id })
      .update({ ...payload, updated_at: this.db.fn.now() })
      .returning('*');
    return rental;
  }

  async getMonthlyReport(monthStart: string, monthEnd: string, vehicleId?: number) {
    const query = this.db(`${this.tableName} as r`)
      .join('vehicles as v', 'v.id', 'r.vehicle_id')
      .whereIn('r.status', ['booked', 'ongoing', 'completed'])
      .andWhere('r.start_date', '<=', monthEnd)
      .andWhere('r.end_date', '>=', monthStart);

    if (vehicleId) query.andWhere('v.id', vehicleId);
    ...
    return query
      ...
      .select(
        'v.id',
        'v.name',
        this.db.raw('COUNT(r.id)::int as total_bookings'),
        this.db.raw(
          `SUM((LEAST(r.end_date::date, ?::date) - GREATEST(r.start_date::date, ?::date)) + 1)::int as days_rented`,
          [monthEnd, monthStart],
        ),
        this.db.raw(
          `SUM(((LEAST(r.end_date::date, ?::date) - GREATEST(r.start_date::date, ?::date)) + 1) * v.daily_rate)::numeric as revenue`,
          [monthEnd, monthStart],
        ),
      );
  }
