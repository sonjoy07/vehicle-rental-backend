...
import { Rental, RentalFilters, CreateRentalDto, UpdateRentalDto, PaginatedResult } from '../types/rental.types';
...
export class RentalService {
    ...
    async list(filters: RentalFilters): Promise<PaginatedResult<Rental>> {
        ...
        return {
            data, total, page: filters.page || 1, limit: filters.limit || 10
        }
    }
    ...
    async update(id: number, payload: UpdateRentalDto,): Promise<Rental> {
        return db.transaction(async (trx) => {
            ...
            if (dateChanged) {
                const clash = await this.rentalDAO.findOverLapping(existing.vehicle_id, startDate, endDate, id, trx);
                if (clash) throw new AppError('Vehicle is already booked for these days', 409)
